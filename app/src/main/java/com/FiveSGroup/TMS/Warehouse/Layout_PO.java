...
public class Layout_PO extends AppCompatActivity implements View.OnClickListener, AdapterView.OnItemSelectedListener {
            ...
            if (!stockinDate.equals("")) {
                ...
                if (checkBoxDate.isChecked()) {
                    ...
                    if(!shelfLife.equals("")){
                        ...
                        if (date_current.compareTo(newDate_curent) > 0) {
                            LayoutInflater factory = LayoutInflater.from(Layout_PO.this);
                            View layout_cus = factory.inflate(R.layout.layout_warn, null);
                            final AlertDialog dialog = new AlertDialog.Builder(Layout_PO.this, R.style.Theme_AppCompat_Light_Dialog_MinWidth).create();
                            dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
                            ColorDrawable back = new ColorDrawable(Color.TRANSPARENT);
                            InsetDrawable inset = new InsetDrawable(back, 64);
                            dialog.getWindow().setBackgroundDrawable(inset);
...
//        }
    ...
    private void IntentActivity(final Class activity, final String type, final String unit) {
        Intent intent = new Intent(Layout_PO.this, activity);
