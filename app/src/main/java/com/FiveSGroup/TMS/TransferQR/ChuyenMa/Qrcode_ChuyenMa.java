...
//            initialiseDetectorsAndSources();
            if (ContextCompat.checkSelfPermission(Qrcode_ChuyenMa.this, Manifest.permission.CAMERA)
                    == PackageManager.PERMISSION_DENIED){
                ActivityCompat.requestPermissions(Qrcode_ChuyenMa.this, new String[] {Manifest.permission.CAMERA}, 123);
            } else {
                startScanning();
            }
        }
            ...
            public void onDecoded(@NonNull final Result result) {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Toast.makeText(Qrcode_ChuyenMa.this, result.getText(), Toast.LENGTH_SHORT).show();
                    }
                });
            }
        });
...
//                        String product_code = product_s_ps.get(vitri).getPRODUCT_CODE();
    ...
    private void getinformation(final String barcodeData) {
        int statusGetCustt = new CmnFns().getQAFromServer(barcodeData, CmnFns.readDataAdmin(), "WTP", 0, global.getChuyenMaCD());
